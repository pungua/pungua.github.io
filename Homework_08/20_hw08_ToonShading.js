...
// const texture = loadTexture(gl, true, '../images/textures/sunrise.jpg'); // 텍스쳐 없음

const cameraPos = vec3.fromValues(0, 0, 3);
const lightDirection = vec3.fromValues(1.0, 0.25, 0.5); // Light 위치 수정
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            toonLevel = 1.0;
            updateText(textOverlay3, "toon levels: " + toonLevel, 3);
        }
        else if (event.key == '2') {
            toonLevel = 2.0;
            updateText(textOverlay3, "toon levels: " + toonLevel, 3);
...
function render() {
    ...
    shader.setFloat("toonLevel", toonLevel); // 툰 레벨 설정
...
async function main() {
    try {
        ...
        // gl.bindTexture(gl.TEXTURE_2D, texture); // 텍스쳐 없음
        shader.setVec3("material.diffuse", vec3.fromValues(1.0, 0.5, 0.31)); // 기본 색상 설정
        ...
        cylinder.copyVertexNormalsToNormals(); // only smooth shading 고정
